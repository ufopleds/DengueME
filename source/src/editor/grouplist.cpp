...
#include "components/populationdata.h"
...
void GroupList::setXml(QDomNode root)
{
    ...
    for (QDomElement node = root.firstChildElement(); !node.isNull(); node = node.nextSiblingElement()) {
        ...
        else if (node.tagName().compare("populationdata", Qt::CaseInsensitive) == 0)  {
...
void GroupList::addPopulation()
{
    addComponent(new PopulationData);
}
